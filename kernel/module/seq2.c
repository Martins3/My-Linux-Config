...
static void stop(struct seq_file *s, void *v) { pr_info("stop\n"); }
...
int simple_seq_init2(void) {
  debugfs_file =
      debugfs_create_file("lkmc_seq_file", S_IRUSR, NULL, NULL, &fops);
  if (debugfs_file) {
    return 0;
  } else {
    return -EINVAL;
...
void simple_seq_fini2(void) { debugfs_remove(debugfs_file); }
