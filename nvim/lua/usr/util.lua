...
local function close_hidden_buffers()
  ...
  -- Session argument lists must not recreate buffers that were closed.
  -- 2026-09-14 这是一个很奇怪的问题，如果当前关闭了 buffer ，但是 session 一打开，结果这些 buffer 都会恢复回来
  for _, win in ipairs(vim.api.nvim_list_wins()) do
    vim.api.nvim_win_call(win, function()
      for i = vim.fn.argc() - 1, 0, -1 do
        if vim.fn.buflisted(vim.fn.argv(i)) == 0 then
